import require$$1$3, { ipcMain, app, desktopCapturer, screen, BrowserWindow } from "electron";
...
const logCrash = (type2, msg) => {
  try {
    const logFilePath = "c:\\Users\\chiva\\Documents\\devMGM\\dynamic_v2\\crash_log.txt";
    fs$j.appendFileSync(logFilePath, `[${(/* @__PURE__ */ new Date()).toISOString()}] [${type2}] ${msg}
`);
  } catch (e) {
  }
};
...
const getResPath = (relPath) => {
  if (app.isPackaged) {
    const prodPath = path$m.join(process.resourcesPath, relPath);
    if (fs$j.existsSync(prodPath)) return prodPath;
    const prodElectronPath = path$m.join(process.resourcesPath, "electron", relPath);
    if (fs$j.existsSync(prodElectronPath)) return prodElectronPath;
    return prodPath;
  }
  const devPath = path$m.join(process.cwd(), relPath);
  if (fs$j.existsSync(devPath)) return devPath;
  const electronPath = path$m.join(process.cwd(), "electron", relPath);
  if (fs$j.existsSync(electronPath)) return electronPath;
  return path$m.join(__dirname$1, "..", relPath);
};
let psNotif = null;
...
const startCalendarMonitor = () => {
  ...
  psCalendar.stdout.on("data", (d) => {
    ...
    for (let line of lines) {
      ...
      if (line.startsWith("__EVENT__")) {
        ...
        if (parts.length >= 6) {
          const [title, start, end, loc, type2, id] = parts;
          eventsFound.push({ title, start, end, loc, type: type2, id });
        }
      }
    }
  ...
  psCalendar.on("exit", () => setTimeout(startCalendarMonitor, 1e4));
};
const startNotifMonitor = () => {
  const notifPath = getResPath("notifications-monitor.ps1");
  if (!fs$j.existsSync(notifPath)) {
...
ipcMain.on("check-for-updates", () => {
  ...
  main$1.autoUpdater.checkForUpdates().then((result) => {
...
ipcMain.on("start-update-download", () => {
  ...
  main$1.autoUpdater.downloadUpdate().then(() => {
...
ipcMain.handle("get-notif-permission-status", async () => {
  return new Promise((resolve) => {
    ...
    exec(`powershell -Command "${cmd}"`, (error2, stdout) => {
      if (error2) {
        console.error("[NOTIF_IPC] Error checking status:", error2);
        resolve("Denied");
      } else {
  ...
  https.get(url, (res) => {
    ...
    res.on("end", () => {
      ...
      try {
        ...
        const current = json2.current_condition[0];
        const area = (_a = json2.nearest_area) == null ? void 0 : _a[0];
...
ipcMain.on("set-is-expanded", (_, expanded) => {
  ...
  if (expanded && Date.now() - lastAutoCheckTime > 30 * 60 * 1e3) {
    ...
    main$1.autoUpdater.checkForUpdatesAndNotify().catch(() => {
    });
  }
});
...
function createWindow() {
  ...
  win.webContents.on("did-finish-load", () => {
    win == null ? void 0 : win.setTitle("NOTCHLY_ALIVE");
...
const pollNetworkStatus = async () => {
  ...
  try {
    ...
    exec(`powershell -Command "${script.replace(/\n/g, " ")}"`, (err, stdout) => {
      ...
      networkInterval = setTimeout(pollNetworkStatus, 6e3);
    });
  } catch (e) {
    if (networkInterval) clearTimeout(networkInterval);
    networkInterval = setTimeout(pollNetworkStatus, 6e3);
...
const getVol = () => new Promise((res) => {
  if (!fs$j.existsSync(volExe)) return res(-1);
...
const pollVol = async () => {
  ...
  volInterval = setTimeout(pollVol, 2e3);
};
let mediaProc = null;
...
const startMediaReader = () => {
  try {
    ...
    const filterLog = (data, prefix) => {
      const str2 = data.toString();
      if (str2.includes("failed to get playback info") || str2.includes("OperaSoftware")) return;
      if (prefix === "ERR") console.error(`[MEDIA_READER_ERR] ${str2.trim()}`);
      else console.log(`[MEDIA_READER] ${str2.trim()}`);
    };
    if (mediaProc.stdout) mediaProc.stdout.on("data", (d) => filterLog(d, "LOG"));
    if (mediaProc.stderr) mediaProc.stderr.on("data", (d) => filterLog(d, "ERR"));
    if (mediaProc) {
...
try {
  ...
  const startMeetPS = () => {
    const psPath = path$m.join(os$1.tmpdir(), "notchly-meet.ps1");
    const psCode = `
  ...
  ipcMain.handle("get-media-source-id", async (_event, mediaInfo) => {
    try {
      ...
      let source = sources.find((s) => {
        ...
        return n.includes(t2) && a.some((x) => n.includes(x));
      });
      if (!source) source = sources.find((s) => s.name.toLowerCase().includes(t2));
...
`;
  ipcMain.handle("toggle-wifi", async () => {
    const psPath = path$m.join(os$1.tmpdir(), "notchly-radio-cmd.ps1");
    fs$j.writeFileSync(psPath, radioControlScript, "utf8");
